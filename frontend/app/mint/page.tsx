    ...
    <>
      ...
      <main className="mx-auto max-w-xl px-4 py-6 pb-24 md:pb-10">
          ...
          {address && (
            <div className="grid grid-cols-4 gap-1 sm:gap-2">
              {resolvedTiers.map((tier) => {
                ...
                if (tier.minted) {
                  return (
                    <div key={tier.id} className="rounded-xl py-2.5 text-center text-xs font-bold border"
                ...
                if (tier.unlocked) {
                  return (
                    <button key={tier.id} onClick={() => handleMint(tier.id)} disabled={isMinting}
                ...
                return (
                  <div key={tier.id} className="rounded-xl py-2.5 text-center text-xs font-semibold border text-[--text-secondary]"
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4 rounded-2xl border border-[--border] bg-[--bg-card] px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[--text-secondary] mt-0.5">
                {xFollowState === "countdown" ? `Verifying in ${xCountdown}s…`
                  : xAlreadyClaimed ? "Reward claimed"
          ...
          />
          <TaskRow
            icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#C9693A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>}
            title="Make 1 Multi Swap" desc="For each completed Multi Swap" pts={150}
          />
          <TaskRow
            ...
            title="Swap with AI Agent" desc="For each transaction approved via x402" pts={250}
          />
          <TaskRow
